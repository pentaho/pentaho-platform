...
see [JcrTemplate exception translation layer](../../architecture/unified-repository/layer-jcr-template-exception-translation.md) for why this extra hop exists (the `JcrTemplate` → `SessionFactoryUtils.translateException()`
translation layer).
...
> |---|---|---|---|
> | `UnifiedRepositoryFileExistsException` | `undeleteFile` | `RepositoryFileDaoFileExistsException` (one hop — this DAO exception is a plain `RuntimeException`, not a `javax.jcr.RepositoryException`, so `JcrTemplate` never touches it, [JcrTemplate exception translation layer](../../architecture/unified-repository/layer-jcr-template-exception-translation.md)) | A file/folder already exists at the deleted item's original path. Not an access-control condition at all — do not run this through not-found/no-write disambiguation logic. |
...
try {
    ...
    // Exactly two hops, per [JcrTemplate exception translation layer](../../architecture/unified-repository/layer-jcr-template-exception-translation.md)/[IUnifiedRepository exception taxonomy](exception-taxonomy.md) — URE -> DataRetrievalFailureException -> JCR
    ...
    // different intermediate class than DataRetrievalFailureException — see [JcrTemplate exception translation layer](../../architecture/unified-repository/layer-jcr-template-exception-translation.md))
}
```

`ExceptionLoggingDecorator` itself uses a similar technique internally
(`ExceptionUtils.getThrowableList(e)`, reversed) to find a matching converter — see [ExceptionLoggingDecorator layer](../../architecture/unified-repository/layer-exception-logging-decorator.md)
and [JcrTemplate exception translation layer](../../architecture/unified-repository/layer-jcr-template-exception-translation.md). That decorator-internal usage walks the full list because it must match
...
every generic `UnifiedRepositoryException` (see [IUnifiedRepository exception taxonomy](exception-taxonomy.md)).
