...
public class UserRoleWebService implements IUserRoleWebService {
  ...
  public boolean createUser(ProxyPentahoUser proxyUser) throws UserRoleException {
    getDao().createUser(proxyUser.getTenant(), proxyUser.getName(), proxyUser.getPassword(), proxyUser.getDescription(), null);
    return true;
  }
  ...
  public boolean updateRoleObject(ProxyPentahoRole proxyPentahoRole) throws UserRoleException {
    ...
    if (role == null) {
      throw new UserRoleException(Messages.getInstance().getErrorString("UserRoleWebService.ERROR_0006_ROLE_UPDATE_FAILED", proxyPentahoRole.getName())); //$NON-NLS-1$
    }
