...
public class CredentialsStrategySessionFactory implements InitializingBean, DisposableBean, SessionFactory {
  ...
  // Session cache by credentials, partitioned by thread. Two threads obtaining sessions for the same credentials cannot use
  // the same Session.
...
//        }
  ...
  // TODO: remove for production, overhead here.
    ...
    if ( !TransactionSynchronizationManager.isSynchronizationActive() ) {
      ...
      try {
        ...
        if ( SessionFactoryUtils.isSessionThreadBound( session, this ) ) {
          if ( LOG.isDebugEnabled() ) {
            LOG.debug( "Session is bound to a transaction. This should never happen, ignoring this session and creating a new session: " + creds );
          }
          sessionCache.invalidate( key );
          session = sessionCache.get( key );
        }
