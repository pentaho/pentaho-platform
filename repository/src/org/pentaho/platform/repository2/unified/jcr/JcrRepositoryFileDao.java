...
public class JcrRepositoryFileDao implements IRepositoryFileDao {
  ...
  private RepositoryFile internalCreateFolder(final Session session, final Serializable parentFolderId,
    ...
    if(parentFolderId != null) {
      ...
      if(parentRepositoryFile != null) {
        ...
        if (accessVoterManager != null && (!accessVoterManager.hasAccess(parentRepositoryFile,
            RepositoryFilePermission.WRITE, parentAcl, PentahoSessionHolder.getSession()))) {
          return null;
        }
      }
    }
  ...
  private RepositoryFile internalUpdateFile(final Session session, final PentahoJcrConstants pentahoJcrConstants,
    ...
    // Invoke accessVoterManager to see if we have access to perform this operation
    if (accessVoterManager != null && (!accessVoterManager.hasAccess(file, RepositoryFilePermission.WRITE, acl, PentahoSessionHolder.getSession()))) {
      return null;
    }
    ...
    return (RepositoryFile) jcrTemplate.execute(new JcrCallback() {
      ...
      public Object doInJcr(final Session session) throws RepositoryException, IOException {
        ...
        if(file != null) {
          RepositoryFileAcl acl = aclDao.getAcl(file.getId());
          // Invoke accessVoterManager to see if we have access to perform this operation
          if (accessVoterManager != null && (!accessVoterManager.hasAccess(file, RepositoryFilePermission.READ, acl, PentahoSessionHolder.getSession()))) {
            return null;
          }
        }
        return file;
    ...
    jcrTemplate.execute(new JcrCallback() {
      ...
      public Object doInJcr(final Session session) throws RepositoryException, IOException {
        ...
        if(fileToBeDeleted != null) {
          ...
          // Invoke accessVoterManager to see if we have access to perform this operation
          if (accessVoterManager != null && (!accessVoterManager.hasAccess(fileToBeDeleted, RepositoryFilePermission.DELETE, toBeDeletedFileAcl, PentahoSessionHolder.getSession()))) {
            return null;
          }
        }
