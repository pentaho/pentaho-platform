...
//  }
  ...
  public void setRoleMembers(Session session, final String tenantName, final String roleName, final String[] memberUserNames) throws RepositoryException, NotFoundException {
    ...
    if(jackrabbitGroup != null) {
      internalSubTenantCheck(tenantedUserNameUtils.getTenantName(jackrabbitGroup.getID()));
    }
  ...
  public void setUserRoles(Session session, final String tenantName, final String userName, final String[] roles) throws RepositoryException, NotFoundException {
      ...
      if(jackrabbitUser != null) {
        internalSubTenantCheck(tenantedUserNameUtils.getTenantName(jackrabbitUser.getID()));
      }
 ...
 public List<IPentahoRole> getRoles(Session session, final String tenant, boolean includeSubtenants) throws RepositoryException {
  ...
  private UserManagerImpl getUserManager(String tenantName, Session session) throws RepositoryException {
    ...
    tenantProperties.put(UserManagerImpl.PARAM_USERS_PATH, UserManagerImpl.USERS_PATH + ((!tenantName.startsWith(RepositoryFile.SEPARATOR)) ? RepositoryFile.SEPARATOR: "") + tenantName);
    tenantProperties.put(UserManagerImpl.PARAM_GROUPS_PATH, UserManagerImpl.GROUPS_PATH + ((!tenantName.startsWith(RepositoryFile.SEPARATOR)) ? RepositoryFile.SEPARATOR: "") + tenantName);      
  ...
  private void internalSubTenantCheck(String tenantPath) {
    if(tenantPath != null) {
      String currentTenantId = (String) PentahoSessionHolder.getSession().getAttribute(IPentahoSession.TENANT_ID_KEY);
      if(!tenantManager.isSubTenant(currentTenantId, tenantPath)) {
        throw new AccessDeniedException(tenantPath + "is not a sub tenant of " + currentTenantId);
      }
    }
  }
}
