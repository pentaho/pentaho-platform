  ...
  public static class JaxBSafeEntry {
    ...
    public JaxBSafeEntry(Map.Entry<String, ParamValue> e) {
      ...
      if (v instanceof StringParamValue) {
        ...
        /*
         * To overcome JAXB nulling out empty lists, we set a flag
         * here so when we read back the value on the receiving side
         * we can properly construct an empty list.
         */
        if(listValue.size() == 0) {
    ...
    public ListParamValue getListValue() {
      if(listValue == null && listValueIsEmptyList) return new ListParamValue();
