...
public interface IScheduler {
  public enum SchedulerStatus {RUNNING, PAUSED, STOPPED};
  
  /**
   * Schedules a job to be run at one or more times in the future.
   * @param jobName the user defined name for the job
   * @param action the action to be run at the scheduled time(s)
   * @param jobParams the parameters to be passed to the action when it runs
   * @param trigger the time(s) at which to run the action
   ...
   * Schedules a job to be run at one or more times in the future.
   * @param jobName the user defined name for the job
   * @param actionId the bean Id of the action to be run as defined an a plugin's plugin.xml
   * @param jobParams the parameters to be passed to the action when it runs
   * @param trigger the time(s) at which to run the action
   * @return the scheduled job
   * @throws SchedulerException If the job could not be scheduled
   ...
   * @param jobParams the parameters to be passed to the action when it runs
   * @param trigger the time(s) at which to run the action
   * @param outputStreamProvider if the action being scheduled expects to write to an output stream, at the time of action execution the this provider will be used to create the stream that passed to the action.
   * @return the scheduled job
   * @throws SchedulerException If the job could not be scheduled
   */
   ...
   * @param jobId the ID of an existing scheduled job
   * @param jobParams the parameters to be passed to the action when it runs
   * @param trigger the time(s) at which to run the action
   * @throws SchedulerException If the job could not be updated
   */
  ...
  /**
   * Prevents the specified job from running in the future. The job remains in the list of scheduled jobs in
   * a "paused" state. If the job is currently running, the currently running job is allowed to complete.
   * @param jobId the job to be paused
   */
   ...
   * @param windows the new scheduler availability map
   */
   ...
   * @param subject the subject whose window is being requested
  ...
  /**
   * Pauses the entire scheduler, which prevents all scheduled jobs from running. Any currently running jobs are allowed to complete.
   * Note that the "paused" state of individual jobs is not changed by this call.
   */
  ...
  /**
   * Shuts the scheduler down so it will process no more jobs.  The implementation will decide if this means kill jobs in progress
   * or let them finish.
   */
  public void shutdown() throws SchedulerException;
  
  /**
   * Sets the minimum time that must elapse between runs of any jobs. For example if set to "5" then
   * a job may not be scheduled to run less than 5 seconds apart.
   * @param subject the subject to which the interval applies
   ...
   * @param subject the subject whose min interval is being requested
   ...
   * @param filter the filter to use to determine which jobs to return. If null all scheduled
   * jobs are return.
   ...
   * @param jobId the job for which to query it's execution history
   ...
   */
  public void fireJobCompleted(final IAction actionBean, final String actionUser, final Map<String, Serializable> params, IBackgroundExecutionStreamProvider streamProvider);
}
