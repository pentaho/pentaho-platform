 ...
 * The scheduling system is responsible for creating {@link Job}s via 
 * {@link IScheduler#createJob(String, Class, Map, JobTrigger)}.  Jobs are
 * likely persistent, at least for the life of a {@link IScheduler} instance.
 * In other words, an {@link IScheduler} instance should never forget about a 
 * Job it has created, unless the Job has been removed via
 * {@link IScheduler#removeJob(Job)}.
 * <p>
 * Note: once the scheduler engine processes a job run, it will create a new
 * {@link IJobResult}, which will contain full historical information about job runs.
 * {@link Job} will contain only minimal of such temporal information. 
 * 
 * @author aphillips
 */
@XmlRootElement
public class Job {
  ...
  /**
   * @return the next time the job is scheduled to run or null if the job will
   * not run again.
   */
   ...
   * @param jobParams the parameters to be passed to the IAction
   */
   ...
   * @param lastRun the last time the job ran. null if the job has not run.
   */
   ...
   * @param nextRun the next time the job will run. null if the job will not run again.
   */
   ...
   * @param schedulableClass the name of the IAction to run.
   */
