 ...
 * efficiently further filtered for specific types of actions, such as self-actions or object-actions.
 * <p>
 * Operations such as {@link #getSelfActions()} and {@link #getObjectActions()} provide convenient access to
 * self-actions and object-actions, respectively.
 * <p>
...
public interface IAuthorizationActionService {
  ...
  // region Self and Object Actions
  /**
  ...
  /**
   * Gets a stream of authorization actions that are object-actions, as determined by
   * {@link IAuthorizationAction#executesOnObjectType(String)}.
   * <p>
   * This method is syntactic sugar for {@code getActions().filter(IAuthorizationAction::isObjectAction)}.
   *
   * @return A stream of {@link IAuthorizationAction} that are object-actions.
   */
  @NonNull
  default Stream<IAuthorizationAction> getObjectActions() {
    return getActions().filter( IAuthorizationAction::isObjectAction );
  }

  /**
   * Gets a stream of authorization actions that can be executed on objects of a specific type.
   * <p>
   * This method is syntactic sugar for {@code getActions().filter(action -> action.executesOnObjectType(objectType))}.
   *
   * @param objectType The object type.
   * @return A stream of {@link IAuthorizationAction} that execute on objects of the specified type.
   * @throws IllegalArgumentException if the object type is {@code null} or empty.
   */
  @NonNull
  default Stream<IAuthorizationAction> getObjectActions( @NonNull String objectType ) {
    if ( StringUtils.isEmpty( objectType ) ) {
      throw new IllegalArgumentException( "Argument `objectType` cannot be null or empty." );
    }

    return getActions().filter( action -> action.executesOnObjectType( objectType ) );
  }
  // endregion
}
