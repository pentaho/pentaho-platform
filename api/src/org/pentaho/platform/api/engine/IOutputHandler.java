...
/**
 * An OutputHandler manages the content generated from a Component execution. Output can take the form of the generated
 * results from a component, or content that solicits additional information from the requester. The handler also
 * manages the relationship with the ActionDefinition and output content validation.
 */
public interface IOutputHandler {
  ...
  /**
   * Indicates whether or not the handler is expected to have data written to a response output stream managed by the
   * handler. Typically, a handler will want to return true here if its getOutputContentItem or setOutput methods have
   * been invoked and their invocations can result in a write to the response output stream that is managed by the
   * handler. In general, handlers are responsible for setting this flag any time a client response is possible.
   * 
  ...
  /**
   * Retrieve the ContentItem that describes the request interface for additional or missing information (missing from
   ...
   * <p>
   * Generally, if there is no client on the other side of the request that could receive and process feedback, then
   * this boolean should be setto false.
   * 
  ...
  /**
   * Sets the output type that is wanted by the handler. Valid values are OUTPUT_TYPE_PARAMETERS, OUTPUT_TYPE_CONTENT,
   * OUTPUT_TYPE_DEFAULT
   * 
   * @param outputType
  ...
  /**
   * Sets an output of the handler. For example the HTTP handler will accept output names of 'header' allowing an HTTP
   * header to be set, and 'redirect' allowing the responses sendRedirect to be called.
   * 
   * @param name
   *          Name of the output
   * @param value
