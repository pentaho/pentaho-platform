...
/**
 * Utility to go between principal ids (user/role ids) and principal name and tenant. For example let's assume that user
 * "admin" belongs to tenant "acme". This resolver class class is responsible for determining the user's unique id
 * within a multi-tenanted environment where multiple users with the same name may exist across tenants. Conversely this
 * class must be able to convert a unique user id to a user name & tenant.
 * 
 * @author rmansoor
 * 
 */
