...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:str="http://www.w3.org/2001/10/str-util.xsl"
                version="2.0">
  ...
  <div xmlns="http://www.w3.org/1999/xhtml">
    ...
    <h2>Status</h2>
    <p>The templates in this stylesheet are used in several other ones. Please don't modify it without letting me know
      and don't break the current exposed API.
    </p>
    <h2>Variables</h2>
    <dl>
      <dt>str:uc</dt>
    ...
    <dl>
      ...
      <dd>Takes a
        ...
        and a
        <var>delimiter</var>
        parameters and returns the part of the string before the first appearance of
        <var>delimiter</var>
        in<var>string</var>, the complete
        <var>string</var>
        if
        <var>delimiter</var>
        is not in<var>string</var>.
      </dd>
      <dt>str:ends-with</dt>
      <dd>Takes a
        <var>string</var>
        ...
        if
        <var>string</var>
        ends with<var>string2</var>, nothing otherwise (please use
        <code>normalize-space()</code>
        on the results to avoid bad surprises
      </dd>
      <dt>str:has-token</dt>
      <dd>Takes a<var>string</var>, a
        <var>token</var>
        and an optinal
        <var>delimiter</var>
        (default at " ") parameters and returns if
        <var>token</var>
        is a token of
        <var>string</var>
        delimited by
        <var>delimiter</var>
        the value<var>token</var>, nothing (that is, an unspecified number of spaces; please use
        <code>normalize-space()</code>
      ...
      <dd>Takes a
        ...
        and a
        <var>delimiter</var>
        parameters and returns the part of the string before the last appearance of
        <var>delimiter</var>
        in<var>string</var>, the complete
        <var>string</var>
        if
    ...
    <hr/>
    <address>$Id: str-util.xsl,v 1.2 2005/11/29 08:15:34 jdixon Exp $
      <br/>
...
$suffix_length + 1))=$string2)">
  ...
  <xsl:template name="str:has-token">
    ...
    <xsl:if
        test="$string=$token or starts-with($string,concat($token,$delimiter)) or contains($string,concat($delimiter,$token,$delimiter)) or (contains($string,concat($delimiter,$token)) and substring($string,string-length($string) - string-length($token) - string-length($delimiter) + 1)=concat($delimiter,$token))">
