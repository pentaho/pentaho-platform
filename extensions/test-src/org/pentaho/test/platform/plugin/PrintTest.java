...
public class PrintTest extends BaseTest {
  ...
  /*
   * public void testPrinting1() { startTest(); IRuntimeContext context = run("test", "printing", "PrintTest1.xaction");
  ...
  /*
   * public void testPrinting_NoPrinterName() { startTest(); IRuntimeContext context = run("test", "printing",
   * "PrintTest_NoPrinterName.xaction"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ assertEquals(
   * Messages.getInstance().getString("BaseTest.USER_RUNNING_ACTION_SEQUENCE"), IRuntimeContext.RUNTIME_STATUS_SUCCESS,
   * context.getStatus()); //$NON-NLS-1$ finishTest(); }
   */
  ...
  public static void main( String[] args ) {
    ...
    try {
      /*
       * test.testPrinting1(); test.testPrinting_NoPrinterName();
       */
