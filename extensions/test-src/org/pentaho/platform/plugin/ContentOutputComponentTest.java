...
public class ContentOutputComponentTest extends BaseTest {
   ...
   * 
   * // Test with an invalid input parameter parameterProvider.setParameter("CONTENTOUTPUT", neverWritten.getBytes());
   * //$NON-NLS-1$ context = run("test", "platform", "ContentOutputTest_Bytearray.xaction", parameterProvider, testName,
   * CO_TEST_EXTN); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ assertEquals(IRuntimeContext.RUNTIME_STATUS_FAILURE,
   * context.getStatus());
   * 
   * finishTest(); }
   */
