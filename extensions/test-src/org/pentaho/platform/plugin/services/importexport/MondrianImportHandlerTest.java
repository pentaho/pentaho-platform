...
public class MondrianImportHandlerTest extends TestCase {
  ...
  protected void setUp() throws Exception {
    ...
    repository.createFolder( repository.getFile( "/etc" ).getId(), new RepositoryFile.Builder( "mondrian" ).folder(
  ...
  /*
   * public void testDoImport() throws Exception { { final MondrianImportHandler handler = new
   * MondrianImportHandler(repository); try { handler.doImport(null, null, null, true);
   ...
   * assertNotNull(repository.getFile("/etc/mondrian/SteelWheels/schema.xml")); } finally { IOUtils.closeQuietly(zis); }
   * }
   */
