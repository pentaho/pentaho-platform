...
public class MetadataImportHandlerTest {
  ...
  public void testDomainWithLocaleFiles() throws Exception {
    ...
    final IPlatformImportBundle bundle =
      new RepositoryFileImportBundle.Builder().input( in ).charSet( "UTF-8" ).hidden( false ).overwriteFile( true )
        .mime( "text/xmi+xml" ).name( "steel-wheels.xmi" ).comment( "Test Metadata Import" ).withParam( "domain-id",
          "steel-wheels" ).addChildBundle( localizationBundle ).addChildBundle( localizationBundle2 ).build();
  ...
  public void testPreserveDsw() throws Exception {
    ...
    try {
      ...
      context.checking( new Expectations() {
        {
          oneOf( metadataImporter ).storeDomain( with( xmiInputStreamHasOlap( true ) ), with( equal( domainId ) ),
            with( equal( true ) ) );
        }
      } );
      importer.importFile( bundle );
  ...
  public void testNotPreserveDsw() throws Exception {
    ...
    try {
      ...
      context.checking( new Expectations() {
        {
          oneOf( metadataImporter ).storeDomain( with( xmiInputStreamHasOlap( false ) ), with( equal( domainId ) ),
            with( equal( true ) ) );
        }
      } );
      importer.importFile( bundle );
  ...
  private RepositoryFileImportBundle.Builder getMetadataImport( final String domainId, final InputStream input ) {
    return new RepositoryFileImportBundle.Builder().input( input ).charSet( "UTF-8" ).mime( "text/xmi+xml" ).hidden(
      false ).overwriteFile( true ).withParam( "domain-id", domainId );
  }
