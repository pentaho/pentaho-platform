...
public class PentahoWebContextFilterTest {
  ...
  public void testWebContextCachedWaitSecondVariable() throws Exception {
    ...
    Mockito.when( cacheManager.getFromGlobalCache( PentahoSystem.WAIT_SECONDS ) ).thenReturn( null )
      .thenReturn( new Integer( 30 ) );

    filter.printRequireJsCfgStart( new ByteArrayOutputStream() );
    filter.printRequireJsCfgStart( new ByteArrayOutputStream() );

    Mockito.verify( cacheManager, Mockito.times( 2 ) ).getFromGlobalCache( Mockito.eq( PentahoSystem.WAIT_SECONDS ) );
    Mockito.verify( cacheManager, Mockito.times( 1 ) ).putInGlobalCache( Mockito.eq( PentahoSystem.WAIT_SECONDS ), Mockito.anyObject() );
