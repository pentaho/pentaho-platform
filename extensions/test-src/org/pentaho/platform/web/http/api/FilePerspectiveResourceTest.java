...
public class FilePerspectiveResourceTest extends JerseyTest {
  ...
  /*
   * @Test public void testRenderThroughContentGenerator() throws PlatformInitializationException { IUnifiedRepository
   * repo = mock( IUnifiedRepository.class ); final String publicFolderId = "123"; final String fileName = "test.junit";
   * doReturn( new RepositoryFile.Builder( publicFolderId, "public" ).folder( true ).build() ).when( repo ).getFile(
   * ClientRepositoryPaths.getPublicFolderPath() ); RepositoryFile f = new RepositoryFile.Builder( fileName ).build();
   * final String path = ClientRepositoryPaths.getPublicFolderPath() + RepositoryFile.SEPARATOR + fileName;
   * RepositoryFile fWithId = new RepositoryFile.Builder( f ).id( "456" ).path( path ).build(); // return null when the
   * file does not exist and then non-null after file creation when( repo.getFile( path ) ).thenReturn( null
   * ).thenReturn( fWithId ); mp.defineInstance( IUnifiedRepository.class, repo );
   * 
   * mp.define( IPluginProvider.class, JUnitContentGeneratorPluginProvider.class ); PentahoSystem.get(
   * IPluginManager.class ).reload();
   * 
   * WebResource webResource = resource();
   * 
   * // write a .junit file final String text = "sometext"; createTestFile( "repo/files/public:test.junit", text );
   * 
   * // get the output of the .junit file (should invoke the content generator) String textResponse = webResource.path(
   * "repos/:public:test.junit/myperspective" ).get( String.class ); assertEquals(
   * "Content generator failed to provide correct output", "hello viewer content generator", textResponse );
   * 
   * verify( repo ).createFile( eq( publicFolderId ), argThat( isLikeFile( new RepositoryFile.Builder( fileName
   * ).build() ) ), argThat( hasData( text.getBytes(), "application/octet-stream" ) ), anyString() ); }
   */
