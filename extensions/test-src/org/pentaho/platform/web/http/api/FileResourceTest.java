...
import static org.pentaho.platform.web.http.api.JerseyTestUtil.assertResponse;
...
public class FileResourceTest extends JerseyTest implements ApplicationContextAware {
  ...
  public void testWriteBinaryFile() throws InterruptedException {
    ...
    ClientResponse response =
      webResource.path( "repo/files/:public:file.bin" ).accept( APPLICATION_OCTET_STREAM ).get( ClientResponse.class );
  ...
  public void testCopyFiles() throws Exception {
    ...
    ClientResponse r =
      webResource.path( "repo/files/" + destFolderPath + "/children" ).accept( TEXT_PLAIN ).put( ClientResponse.class,
        file.getId() );
  ...
  public void testGetWhenFileDNE() {
    ...
    try {
      ClientResponse r =
        webResource.path( "repo/files/public:thisfiledoesnotexist.txt" ).accept( TEXT_PLAIN )
  ...
  /*
   * @Test public void testBrowserDownload() { final String text = "abcdefg"; // stub IUnifiedRepository start
   * IUnifiedRepository repo = mock( IUnifiedRepository.class );
   * 
   * // mock converters map StreamConverter streamConverter = new StreamConverter( repo ); Map<String, Converter>
   * converterMap = new HashMap(); converterMap.put( "txt", streamConverter );
   * 
   * // stub DefaultExportProcessor DefaultExportHandler defaultExportHandler = new DefaultExportHandler();
   * defaultExportHandler.setConverters( converterMap ); defaultExportHandler.setRepository( repo );
   * 
   * // mock session for getting name to populate manifest info IPentahoSession pentahoSession = mock(
   * IPentahoSession.class ); when( pentahoSession.getName() ).thenReturn( "admin" ); PentahoSessionHolder.setSession(
   * pentahoSession );
   * 
   * final String fileId = "456"; final String fileName = "file.txt"; final String path = "/public/" + fileName;
   * doReturn( new RepositoryFile.Builder( fileId, fileName ).path( path ).build() ).when( repo ).getFile( path ); when(
   * repo.getDataForRead( fileId, SimpleRepositoryFileData.class ) ) .thenReturn( new SimpleRepositoryFileData( new
   * ByteArrayInputStream( text.getBytes() ), null, APPLICATION_OCTET_STREAM ) ) .thenReturn( new
   * SimpleRepositoryFileData( new ByteArrayInputStream( text.getBytes() ), null, APPLICATION_OCTET_STREAM ) ); // stub
   * IUnifiedRepository end
   * 
   * // set object in PentahoSystem mp.defineInstance( IUnifiedRepository.class, repo ); mp.defineInstance(
   * DefaultExportHandler.class, defaultExportHandler );
   * 
   * // test download of file WebResource webResource = resource(); ClientResponse r = webResource.path(
   * "repo/files/public:file.txt/download" ).get( ClientResponse.class );
   * 
   * // test download of dir as a zip file ClientResponse r2 = webResource.path( "repo/files/public:file.txt/download"
   * ).get( ClientResponse.class ); assertResponse( r2, Status.OK ); assertResponseIsZip( r2 ); }
   */

  // We should be testing the underlying class functionality in unit tests
  /*
   * @Test public void testGetDirChildren() { loginAsRepositoryAdmin(); ITenant systemTenant =
   * tenantManager.createTenant( null, ServerRepositoryPaths.getPentahoRootFolderName(), adminAuthorityName,
   * authenticatedAuthorityName, "Anonymous" ); userRoleDao.createUser( systemTenant, sysAdminUserName, "password", "",
   * new String[] { adminAuthorityName } ); ITenant mainTenant_1 = tenantManager.createTenant( systemTenant,
   * MAIN_TENANT_1, adminAuthorityName, authenticatedAuthorityName, "Anonymous" ); userRoleDao.createUser( mainTenant_1,
   * "admin", "password", "", new String[] { adminAuthorityName } ); login( "admin", mainTenant_1, new String[] {
   * authenticatedAuthorityName } ); // stub IUnifiedRepository start IUnifiedRepository repo = mock(
   * IUnifiedRepository.class ); RepositoryFileTree tree = new RepositoryFileTree( new RepositoryFile.Builder( "123",
   * "public" ).build(), Arrays .asList( new RepositoryFileTree( new RepositoryFile.Builder( "123", "public" ).build(),
   * new ArrayList<RepositoryFileTree>( 0 ) ) ) ); doReturn( tree ).when( repo ).getTree( eq(
   * ClientRepositoryPaths.getPublicFolderPath() ), anyInt(), anyString(), anyBoolean() ); // stub IUnifiedRepository
   * end
   * 
   * // set object in PentahoSystem mp.defineInstance( IUnifiedRepository.class, repo );
   * 
   * WebResource webResource = resource(); ClientResponse response = webResource.path( "repo/files/public/children"
   * ).accept( APPLICATION_XML ).get( ClientResponse.class );
   * 
   * assertResponse( response, Status.OK, APPLICATION_XML );
   * 
   * // DOMSourceReader dom = response.getEntity(DOMSourceReader.class); String xml = response.getEntity( String.class
   * ); assertTrue( xml.startsWith( "<?" ) ); cleanupUserAndRoles( mainTenant_1 ); cleanupUserAndRoles( systemTenant );
   * }
   */
  ...
  public void testFileAcls() throws InterruptedException {
    ...
    RepositoryFileAclDto fileAcls =
      webResource.path( "repo/files/public:aclFile.txt/acl" ).accept( APPLICATION_XML )
    ...
    assertEquals( tenantedRoleNameUtils.getPrincipleId( mainTenant_1, authenticatedAuthorityName ), ace.getRecipient() );
