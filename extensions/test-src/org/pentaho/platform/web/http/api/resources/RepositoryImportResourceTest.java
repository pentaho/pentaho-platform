...
public class RepositoryImportResourceTest {
  ...
  public void setUp() throws ObjectFactoryException, PlatformImportException, DomainIdNullException, DomainAlreadyExistsException, DomainStorageException, IOException {
    ...
    Mockito.doReturn( "sampleSession" ).when( session ).getName();
    PentahoSessionHolder.setSession( session );
    handler = Mockito.mock( SolutionImportHandler.class );
    importer = Mockito.mock( PentahoPlatformImporter.class );
    policy = Mockito.mock( IAuthorizationPolicy.class );
    ITenant tenat = Mockito.mock( ITenant.class );
    resolver = Mockito.mock( ITenantedPrincipleNameResolver.class );
    Mockito.doReturn( tenat ).when( resolver ).getTenant( Matchers.anyString() );
    Mockito.doReturn( REAL_USER ).when( resolver ).getPrincipleName( Matchers.anyString() );
    policy = Mockito.mock( IAuthorizationPolicy.class );
    pentahoObjectFactory = Mockito.mock( IPentahoObjectFactory.class );
    iPlatformMimeResolver = Mockito.mock( NameBaseMimeResolver.class );
    iRepositoryImportLogger = Mockito.mock( IRepositoryImportLogger.class );
    catalogService = Mockito.mock( MondrianCatalogHelper.class );
    Mockito.doReturn( "xml" ).when( iPlatformMimeResolver ).resolveMimeForFileName( "" );
    Mockito.doReturn( iRepositoryImportLogger ).when( importer ).getRepositoryImportLogger();
    //for calling importFile in RepositoryImportResource
    Mockito.doAnswer(
        new Answer<Void>() {
          @Override
          public Void answer( InvocationOnMock invocation ) throws Throwable {
            handler.importFile( Matchers.any( IPlatformImportBundle.class ) );
            return null;
          }
        } ).when( importer ).importFile( Matchers.any( IPlatformImportBundle.class ) );
    ...
    Mockito.doAnswer(
        ...
        } ).when( handler ).importFile( Matchers.any( IPlatformImportBundle.class ) );
    ...
    Mockito.doAnswer(
        new Answer<Void>() {
          ...
          public Void answer( InvocationOnMock invocation ) throws Throwable {
            ...
            importsession.setManifest( Mockito.mock( ExportManifest.class ) );
            return null;
          }
  ...
  public void doPostImport() throws ParseException, PlatformImportException {
    ...
    Mockito.when( policy.isAllowed( Matchers.anyString() ) ).thenAnswer( new Answer<Boolean>() {
      @Override
