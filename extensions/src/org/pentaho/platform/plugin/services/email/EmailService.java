...
public class EmailService implements IEmailService {
  ...
  /**
   * The file which contains the email configuration information. This is guaranteed to be NOT NULL by the setEmailConfigFile() method
   */
  ...
  /**
   * Tests the provided email configuration by sending a test email. This will just indicate that the server configuration is correct and a test email was
   * successfully sent. It does not test the destination address.
   * 
   * @param emailConfig
  ...
  protected void setEmailConfigFile(final File emailConfigFile) throws IllegalArgumentException {
    ...
    if (emailConfigFile == null) {
      throw new IllegalArgumentException(messages.getErrorString("EmailService.ERROR_0001_INVALID_CONFIG_FILE_LOCATION", (Object) null));
    }
    if (emailConfigFile.exists()) {
      if (emailConfigFile.isDirectory()) {
        throw new IllegalArgumentException(messages.getErrorString("EmailService.ERROR_0001_INVALID_CONFIG_FILE_LOCATION", emailConfigFile.getAbsolutePath()));
      }
    } else {
  ...
  public boolean isValid() {
    try {
      ...
      if (!StringUtils.isEmpty(c.getSmtpHost()) && !StringUtils.isEmpty(c.getSmtpProtocol()) && !StringUtils.isEmpty(c.getDefaultFrom())) {
        if (c.isAuthenticate()) {
