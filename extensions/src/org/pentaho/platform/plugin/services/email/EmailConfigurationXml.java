...
public class EmailConfigurationXml extends EmailConfiguration {
  ...
  private void loadFromConfigurationDocument(final Document doc) throws DocumentException {
    ...
    if ((rootElement != null) && !doc.getRootElement().getName().equals(ROOT_ELEMENT)) {
      throw new DocumentException(messages
          .getErrorString("EmailConfigurationXml.ERROR_0002_INVALID_ROOT_ELEMENT")); //$NON-NLS-1$
    }
  ...
  private static String getStringValue(final Document doc, final String xpath) {
    try {
      ...
      logger.error(messages.getErrorString("EmailConfigurationXml.ERROR_0001_ERROR_PARSING_DATA", e.getLocalizedMessage()));
    }
    return null;
  }
  ...
  public static Document getDocument(final EmailConfiguration emailConfiguration) {
    ...
    setValue(document, USE_SSL_XPATH, ObjectUtils.toString(emailConfiguration.isUseSsl(), Boolean.FALSE.toString()));
