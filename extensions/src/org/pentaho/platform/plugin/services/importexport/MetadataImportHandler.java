...
public class MetadataImportHandler implements ImportHandler {
  ...
  protected org.pentaho.platform.plugin.services.metadata.IPentahoMetadataDomainRepositoryImporter
  getMetadataImporter() {
    if ( metadataImporter == null ) {
      metadataImporter = new org.pentaho.platform.plugin.services.metadata.
  ...
  protected String processMetadataFile( final ImportSource.IRepositoryFileBundle bundle, final String filename,
    ...
    try {
      ...
      if ( !StringUtils.isEmpty( info.getDomainId() ) ) {
        log.debug( "importing [" + filename + "] as metadata [domain=" + info.getDomainId() + " : overwrite="
            + overwrite + "]" );
      ...
      final String errorMessage =
          messages.getErrorString( "MetadataImportHandler.ERROR_0001_IMPORTING_METADATA", info.getPath(), info
              .getDomainId(), e.getLocalizedMessage() );
      log.error( errorMessage, e );
    }
    return null;
  }

  private void processLocaleFile( final ImportSource.IRepositoryFileBundle file, final PentahoMetadataFileInfo info,
      final boolean overwrite ) {
    try {
      log.debug( "Importing [" + info.getPath() + "] as properties - [domain=" + info.getDomainId() + " : locale="
          + info.getLocale() + "]" );
      if ( this.repository != null ) {
        getMetadataImporter().addLocalizationFile( info.getDomainId(), info.getLocale(), file.getInputStream(),
          overwrite );
      } else {
      ...
      final String errorMessage =
          messages.getErrorString( "MetadataImportHandler.ERROR_0002_IMPORTING_LOCALE_FILE", info.getPath(), info
              .getDomainId(), info.getLocale(), e.getLocalizedMessage() );
      log.error( errorMessage, e );
    }
  }
  ...
  private void addLocalizationFile( String domainId, String locale, InputStream propertiesFile ) throws Exception {
    ...
    String storeDomainUrl =
        this.url + "/plugin/data-access/api/metadata/addLocalizationFile?domainId=" + domainId + "&locale=" + locale;
    WebResource resource = client.resource( storeDomainUrl );
