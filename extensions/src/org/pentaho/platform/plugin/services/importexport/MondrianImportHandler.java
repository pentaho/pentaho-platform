...
public class MondrianImportHandler implements ImportHandler {
  ...
  private void processSchema( ImportSource.IRepositoryFileBundle mondrianFile,
    ...
    if ( repository != null ) {
      org.pentaho.platform.plugin.services.importexport.legacy.MondrianCatalogRepositoryHelper helper =
          new org.pentaho.platform.plugin.services.importexport.legacy.MondrianCatalogRepositoryHelper( repository );
  ...
  /*
   * Parses the datasources.xml and if the current file is referenced in a <Definition> it returns the <DataSourceInfo>
   */
  private String getDatasourcesXMLDatasourceInfo( ImportSource.IRepositoryFileBundle datasourcesXML,
    ...
    if ( datasourcesXML != null ) {
      final String fullFileName =
          RepositoryFilenameUtils.separatorsToRepository( RepositoryFilenameUtils.concat( file.getPath(), file
              .getFile().getName() ) );
  ...
  private NodeList getElementsByTagName( ImportSource.IRepositoryFileBundle documentSource, String tagName )
    throws Exception {
  ...
  private void addSchema( InputStream mondrianFile, String catalogName, String datasourceInfo ) throws Exception {
    ...
    String storeDomainUrl =
        this.url + "/plugin/data-access/api/mondrian/putSchema?catalogName=" + catalogName + "&datasourceInfo="
            + datasourceInfo;
