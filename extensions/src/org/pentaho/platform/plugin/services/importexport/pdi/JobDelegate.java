  ...
  public DataNode elementToDataNode( final RepositoryElementInterface element, final List<Character> reservedChars )
    ...
    if ( log.isDetailed() ) {
      log.logDetailed( toString(),
        "Saving " + jobMeta.nrJobEntries() + " Job enty copies to repository..." ); //$NON-NLS-1$ //$NON-NLS-2$
    }
    ...
    for ( int i = 0; i < jobMeta.nrJobEntries(); i++ ) {
      ...
      DataNode copyNode =
        entriesNode.addNode(
          RepositoryFilenameUtils.escape( copy.getName(), reservedChars ) + "_" + ( i + 1 ) //$NON-NLS-1$
            + EXT_JOB_ENTRY_COPY );
      ...
      // Save the entry information here as well, for completeness. TODO: since this slightly stores duplicate
      // information, figure out how to store this separately.
      //
