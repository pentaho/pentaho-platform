...
public class TransformationDelegate extends AbstractDelegate implements java.io.Serializable {
  ...
  private static final String PROP_STEP_ERROR_HANDLING_DESCRIPTIONS_VALUENAME =
    "step_error_handling_descriptions_valuename"; //$NON-NLS-1$
  ...
  public void dataNodeToElement( final DataNode rootNode, final RepositoryElementInterface element )
    ...
    for ( DataNode stepNode : stepsNode.getNodes() ) {
      ...
      if ( stepPlugin != null ) {
        ...
        throw new KettlePluginLoaderException( stepType, BaseMessages.getString( PKG,
  ...
  protected void loadTransformationDetails( final DataNode rootNode,
                                            final TransMeta transMeta ) throws KettleException {
  ...
  public DataNode elementToDataNode( final RepositoryElementInterface element, final List<Character> reservedChars )
    throws KettleException {
    TransMeta transMeta = (TransMeta) element;
    ...
    for ( StepMeta step : transMeta.getSteps() ) {
      ...
      DataNode stepNode =
        stepsNode.addNode(
          RepositoryFilenameUtils.escape( step.getName(), reservedChars ) + "_" + stepNr + EXT_STEP ); //$NON-NLS-1$
  ...
  private void saveTransformationDetails( final DataNode rootNode, final TransMeta transMeta ) throws KettleException {
    ...
    rootNode
      .setProperty( PROP_STEP_PERFORMANCE_CAPTURING_SIZE_LIMIT, transMeta.getStepPerformanceCapturingSizeLimit() );
  ...
  public SharedObjects loadSharedObjects( final RepositoryElementInterface element,
                                          final Map<RepositoryObjectType, List<? extends SharedObjectInterface>>
                                            sharedObjectsByType )
    throws KettleException {
    TransMeta transMeta = (TransMeta) element;
    ...
    readClusters( transMeta, true,
      (List<ClusterSchema>) sharedObjectsByType.get( RepositoryObjectType.CLUSTER_SCHEMA ) );
