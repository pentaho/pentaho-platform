 ...
 *   		the log and supply it with an output stream to hold the log
 *   		and the root folder of the import.
 *   
 *   2) Call setCurrentFilePath() each time you start processing a new
 *   		import file.  The log shows the file being imported so it must
 *   		be registered.
 *   
 *   3) Call endJob() when the import is done to log the finish and
 *   		release resources.  If the the import terminates abnormally
 *   		this call should be in the finally block.
 *   
 *   
 * Sample code taken from RepositoryImportLogTest
 * 
 * FileOutputStream fileStream = new FileOutputStream(outputFile);
			logger = PentahoSystem.get(IRepositoryImportLogger.class);
			
			//You must call this method to start posting the log.
			logger.startJob(fileStream, "/import/Path");
			
			logger.setCurrentFilePath("/path/file1path");
			logger.debug("Some more detail here");
			logger.info("Success");
...
public interface IRepositoryImportLogger extends Log {
	 ...
	 * the path to the file being processed.  Call this method just
	 * before processing the next file.  It will automatically post
	 * a "Start File Import" entry in the log.
	 * 
	...
	/**
	 * Makes an "End Import Job" log entry and releases memory
	 * associated with this log.
	 */
	void endJob();

	/**
	 * Log informational data.  Should be called when the starting a new file
	 * and when finishing that file.
	 * 
