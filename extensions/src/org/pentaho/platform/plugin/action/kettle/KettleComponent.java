    ...
    try {
      if (isDefinedInput(KettleComponent.DIRECTORY)) {
        ...
        try {
          ...
          error(Messages.getInstance().getErrorString("Kettle.ERROR_0015_BAD_RESOURCE", KettleComponent.TRANSFORMFILE, fileAddress), e); //$NON-NLS-1$
          return false;
        }
      ...
      if (transMeta != null) {
        try {
          ...
          transMeta.clear();
        } catch (Exception ignored){}; // Can't do anything about an exception here.
        transMeta = null;
      }
      if (jobMeta != null) {
        try {
  ...
  private TransMeta loadTransformFromRepository(final String directoryName, final String transformationName, final Repository repository) {
    ...
    try {
      ...
      try {
        ...
        error(Messages.getInstance().getErrorString("Kettle.ERROR_0009_TRANSFROMATION_METADATA_NOT_FOUND", directoryName + "/" + transformationName), e); //$NON-NLS-1$ //$NON-NLS-2$
        return null;
      }
      if (transMeta == null) {
  ...
  private JobMeta loadJobFromRepository(final String directoryName, final String jobName, final Repository repository) {
    ...
    try {
      ...
      try {
        ...
        error(Messages.getInstance().getErrorString("Kettle.ERROR_0020_JOB_METADATA_NOT_FOUND", directoryName + "/" + jobName), e); //$NON-NLS-1$ //$NON-NLS-2$
        return null;
      }
      if (jobMeta == null) {
        error(Messages.getInstance().getErrorString("Kettle.ERROR_0020_JOB_METADATA_NOT_FOUND", directoryName + "/" + jobName)); //$NON-NLS-1$ //$NON-NLS-2$
        debug(getKettleLog(true));
        return null;
      } else {
        return jobMeta;
