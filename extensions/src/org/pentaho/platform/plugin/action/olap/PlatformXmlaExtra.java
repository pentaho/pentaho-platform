...
public class PlatformXmlaExtra implements XmlaHandler.XmlaExtra {
  ...
  public static XmlaHandler.XmlaExtra unwrapXmlaExtra( final OlapConnection olap4jConn )
    ...
    try {
      if ( inCurrentClassLoader( olap4jConn ) ) {
        ...
        return new PlatformXmlaExtra( olap4jConn );
      }
  ...
  private static final String XMLA_EXTRA_CP = "mondrian.xmla.XmlaHandler$XmlaExtra";

  @SuppressWarnings( "unchecked" )
  public Object invoke( String methodName, Class<?> paramType, Object param, Object defaultValue ) {
    try {
      Class<?> clazz = olap4jConn.getClass().getClassLoader().loadClass( XMLA_EXTRA_CP );
      Object xe = olap4jConn.unwrap( clazz );
      return clazz.getMethod( methodName,  paramType ).invoke( xe, param );
    } catch ( Exception e ) {
      e.printStackTrace();
      return defaultValue;
    }
