...
//    String modelType = (String) inputs.get("modeltype");
    ...
    if ( executor == null ) {
      ...
      Class clazz;
      try {
        clazz = Class.forName( queryExecDefault, true,
            queryObject.getLogicalModel().getPhysicalModel().getClass().getClassLoader() );
    ...
    if( executor == null ) {
      ...
      try {
        ...
        logger.error(Messages.getInstance().getErrorString("MetadataQueryComponent.ERROR_0002_NO_EXECUTOR",queryExecName)); //$NON-NLS-1$
        return false;
      }
    }
