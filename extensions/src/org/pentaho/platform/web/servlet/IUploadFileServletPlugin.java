 ...
 * which is not currently supported in {@link IContentGenerator}.
 * @author aphillips
 */
public interface IUploadFileServletPlugin {
  /**
   * A path where the uploaded files will be saved,
   * relative to the solution path.
  ...
  /**
   * The maximum number of files allowed in the target
   * folder.
  ...
  /**
   * The file extension to append to the randomly generated
   * filename resulting from the file upload.
   * @return  the file extension (must explicitly include the '.' character)
   */
  public String getFileExtension();
  
  /**
   * A callback invoked upon successful file upload.  The {@link UploadFileServlet}
   * can do whatever it needs to with the repsonse which will be the actual 
   * servlet response.
   * @param pathToFile  the absolute path to the file successfully uploaded
   * @param response  the servlet response when the file upload succeedes
   */
