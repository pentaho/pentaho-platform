...
public class GwtRpcPluginProxyServlet extends AbstractGwtRpcProxyServlet {
  ...
  protected Object resolveDispatchTarget(String servletContextPath) {
    ...
    if (null == serviceManager.getServiceConfig("gwt", key)) { //$NON-NLS-1$
      String errMsg = Messages.getInstance().getErrorString(
          "GwtRpcPluginProxyServlet.ERROR_0001_SERVICE_NOT_FOUND", key); //$NON-NLS-1$
      logger.error(errMsg);
   ...
   * @param appContextPath according to the example url, this would be '/pentaho'
   * @param servletContextPath according the the example url, this would be '/content/data-access/resources/gwt/{strongName}'
   * @return a URL to the serialization policy file, or <code>null</code> if none applies, 
   * in which case the default serialization policy will apply
   */
  ...
  protected SerializationPolicy doGetSerializationPolicy(HttpServletRequest request, String moduleBaseURL,
     ...
     * The request path is broken up for processing like so:
     * Ex: given a request for serialization file at '/pentaho/content/data-access/resources/gwt/{strongName}'
     *  * modulePath = ???? '/pentaho/content/data-access/resources/gwt/{strongName}'
  ...
  protected String scrubWebAppRoot(String modulePath) {
      //http://localhost:8080/pentaho/api/repo/files/public/pentaho-solutions/steel-wheels/analysis/WEBAPP_ROOT/content/data-access/resources/gwt/
      ...
      if (matcher.find()) {
        ...
        final String rewrittenModulePath = new StringBuffer(modulePath).replace(idx, idx+garbagePathPart.length(), contextPath).toString();
