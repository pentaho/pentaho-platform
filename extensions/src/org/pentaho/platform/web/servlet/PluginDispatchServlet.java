...
/**
 * Dispatches requests to Servlets provided by BIServer plugins.  To define a Servlet
 * in a plugin, simply add a bean element for the Servlet class in the plugin.spring.xml
 * file in your plugin root directory.
 * 
...
public class PluginDispatchServlet implements Servlet {
   ...
   * beyond the servlet base path.  I.e. if the PluginDispatchServlet is mapped to the "/plugin"
   * context in web.xml, then this method will return "myPlugin/myServlet" given a request to 
   * "http://localhost:8080/pentaho/plugin/myPlugin/myServlet".
  ...
  private synchronized void doInit() throws ServletException {
    ...
    for (Map.Entry<String, ListableBeanFactory> pluginBeanFactoryEntry : pluginBeanFactoryMap.entrySet()) {
      ...
      Map<String, Object> beans = BeanFactoryUtils.beansOfTypeIncludingAncestors(pluginBeanFactoryEntry.getValue(), Servlet.class, true, true);
