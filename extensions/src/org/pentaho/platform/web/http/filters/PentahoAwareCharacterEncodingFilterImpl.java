 ...
 * @author mlowery
 */
public class PentahoAwareCharacterEncodingFilterImpl extends SetCharacterEncodingFilter {

  private static final Log logger = LogFactory.getLog( PentahoAwareCharacterEncodingFilterImpl.class );
  ...
  protected String selectEncoding( final ServletRequest request ) {
    ...
    if ( StringUtils.isNotBlank( enc ) ) {
      if ( logger.isDebugEnabled() ) {
        logger.debug( Messages.getInstance().getString(
          "ENCODING_IN_FILTER_INIT", enc ) ); //$NON-NLS-1$
      }
      return enc;
    } else {
      if ( logger.isWarnEnabled() ) {
        logger.warn( Messages.getInstance().getString(
            "COULD_NOT_FIND_ENCODING", //$NON-NLS-1$
