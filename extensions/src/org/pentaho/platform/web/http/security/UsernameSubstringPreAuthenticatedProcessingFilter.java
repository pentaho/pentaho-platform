...
public class UsernameSubstringPreAuthenticatedProcessingFilter extends J2eePreAuthenticatedProcessingFilter {
  ...
  /**
   * Regular expression where the first capture group will be extracted and used as the username. Set to {@code null}
   * or empty string to disable matching and extraction.
   * 
   * <p>
   * Example: <pre>{@code .+\\(.+)}</pre> The above example regular expression would extract {@code USER} from 
   * {@code DOMAIN\USER}. 
   */
