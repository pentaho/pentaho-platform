...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" xmlns:html="http://www.w3.org/TR/REC-html40" exclude-result-prefixes="html">
		...
		<div style="display:block">
			...
			<table width="100%" cellspacing="0" cellpadding="0" class="container_inset_open">
				...
				<tr>
					...
					<td rowspan="2" nowrap="nowrap" class="container_head_right1" title="{view_id}">
						<a>
	...
	<xsl:template name="createColumn">
		<tr id="{column_id}" title='{column_id}' class="unselectedItem" onmousedown="gCtrlr.wiz.getPg( 1 ).getAvailableItemsCtrl().handleItemMouseDown( this, event, '{../view_id}', '{column_id}' );" onmouseup="gCtrlr.wiz.getPg( 1 ).getAvailableItemsCtrl().handleItemMouseUp( this, event, '{../view_id}', '{column_id}' );">
			<td>
