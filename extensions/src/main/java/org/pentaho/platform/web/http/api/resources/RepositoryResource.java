...
/**
 * The RepositoryResource service retrieves the repository files through various methods.  Allows you to execute
 * repository content.
 */
@Path( "/repos" )
public class RepositoryResource extends AbstractJaxRSResource {
  ...
  @NonNull
  protected final IUnifiedRepository repository;
  ...
  public RepositoryResource( @Nullable IUnifiedRepository repository, @Nullable IPluginManager pluginManager ) {
    this.repository = repository != null ? repository : PentahoSystem.get( IUnifiedRepository.class );
    this.pluginManager = pluginManager != null ? pluginManager : PentahoSystem.get( IPluginManager.class );
  }
   ...
   * When set to <code>null</code>, CSRF validation is disabled for all <code>/api/repos</code> content generators.
   * Although, note, it is generally preferable to disable individual content generators, via configuration,
   * by using their operation identifier.
   *
  ...
  @NonNull
  protected FileResource getFileResource() {
    FileResource fileResource = new FileResource();
    fileResource.setHttpServletRequest( httpServletRequest );
    fileResource.setHttpServletResponse( httpServletResponse );
  ...
  public Response doGetFileOrDir( @PathParam( "pathId" ) String pathId ) throws FileNotFoundException {
    return getFileResource().doGetFileOrDir( pathId );
  }
  ...
  /**
   * Gets a resource identified by the compound key contextId and resourceId. This request may include additional
   * parameters used to render the resource.
   *
   * <p><b>Example Request:</b><br />
   ...
   * </p>
   *
   * @param contextId  Identifies the context in which the resource should be retrieved. This value may be a
   *                   repository file ID, repository file extension or plugin ID
   * @param resourceId Identifies a resource to be retrieved. This value may be a static file residing in a publicly
   *                   visible plugin folder, repository file ID or content generator ID
   * @param formParams Any arguments needed to render the resource
   * @return A jax-rs Response object with the appropriate status code, header, and body. In many cases this will
   * trigger a streaming operation after it it is returned to the caller..
   *
   * <p><b>Example Response:</b></p>
   * <pre function="syntax.xml">
   *    This response does not contain data.
  ...
  @StatusCodes( {
    ...
    @ResponseCode( code = 403, condition = "Forbidden. Only trusted origins can access a mutation operation." ),
    @ResponseCode( code = 404, condition = "Failed to find the resource." )
  } )
  public Response doFormPost( @PathParam( "contextId" ) String contextId, @PathParam( "resourceId" ) String resourceId,
    ...
    IOException, URISyntaxException, ServletException {
  ...
  public Response doGet( @PathParam( "contextId" ) String contextId, @PathParam( "resourceId" ) String resourceId )
    throws ObjectFactoryException, PluginBeanException, IOException, URISyntaxException, ServletException {
  ...
  protected Response doService( String contextId, String resourceId ) throws ObjectFactoryException,
    PluginBeanException, IOException, URISyntaxException, ServletException, WebApplicationException {
  ...
  protected Response getContentGeneratorResponse( CGFactory fac )
    ...
    rsc( "Yep, [{0}] is a content generator ID. Executing (where command path is {1}).",
      fac.getContentGeneratorId(),
      fac.getCommand() ); //$NON-NLS-1$

    GeneratorStreamingOutput gso = fac.getStreamingOutput( contentGenerator );

    // Throws WebApplicationException when invalid.
    validateCsrf( contentGenerator, fac.getCommand(), gso );
  ...
  protected void validateCsrf( @NonNull IContentGenerator contentGenerator,
    ...
    if ( csrfValidator != null ) {
      ...
      if ( contentGenerator instanceof IServiceOperationAwareContentGenerator ) {
        contentGenerator.setParameterProviders( gso.getParameterProviders() );
  ...
  protected Response getRepositoryFileResponse( String filePath, String relPath ) throws IOException {
    ...
    Response response =
      fileResource.doGetFileOrDir( RepositoryPathEncoder.encodeRepositoryPath( path ).substring( 1 ) );
