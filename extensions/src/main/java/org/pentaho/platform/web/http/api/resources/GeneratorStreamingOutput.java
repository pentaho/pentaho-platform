...
public class GeneratorStreamingOutput {
   ...
   * <p>
   * This method uses the default plugin manager obtained from the {@link PentahoSystem}.
   *
   * @param contentGenerator     the content generator to invoke
   * @param desc                 a descriptor detailing info about the content generator
   * @param request              the HTTP request
   * @param response             the HTTP response
   * @param acceptableMediaTypes the list of acceptable media types
   * @param file                 the repository file being rendered (can be null if a repository file does not apply)
   * @param command              the trailing part of the URL path of the request, typically used as a command
  ...
  public GeneratorStreamingOutput( IContentGenerator contentGenerator, ContentGeneratorDescriptor desc,
    ...
    this( contentGenerator, desc, request, response, acceptableMediaTypes, file, command, null );
  }
   ...
   * @param command              the trailing part of the URL path of the request, typically used as a command
   *                             sequence (can be null)
   * @param pluginMgr            the Plugin Manager or <code>null</code>, to use the instance provided by the
   * {@link PentahoSystem}.
   */
  public GeneratorStreamingOutput( IContentGenerator contentGenerator, ContentGeneratorDescriptor desc,
                                   HttpServletRequest request, HttpServletResponse response,
                                   List<MediaType> acceptableMediaTypes,
                                   RepositoryFile file, String command, IPluginManager pluginMgr ) {
    ...
    this.file = file;
    mimeTrace(
      "Request is requiring content generator to return content of type [{0}]",
      acceptableMediaTypes.toString() );
    this.acceptableMediaTypes = acceptableMediaTypes;
    this.httpServletRequest = request;
    this.httpServletResponse = response;
    this.pluginMgr = pluginMgr != null ? pluginMgr : PentahoSystem.get( IPluginManager.class );
    ...
    if ( command != null && command.contains( "." ) ) {
      ...
      if ( tmpMimeType != null ) {
        ...
        mimeTrace( "Setting response mime type to [{0}] (based on extension of resource [{1}])", mimeType,
          command );
      }
    }
  }
  ...
  /**
   * Gets the parameter providers which will be used to invoke the content generator.
   * @return The parameter providers map.
   */
  public Map<String, IParameterProvider> getParameterProviders() throws IOException {
    if ( parameterProviders == null ) {
      parameterProviders = createParameterProviders();
    }
  ...
  protected void generateContent( OutputStream outputStream, final MimeTypeCallback callback ) throws Exception {
      ...
      public void setMimeType( String mimeType ) {
        try {
          if ( !MIMETYPE_MUTABLE && GeneratorStreamingOutput.this.getMimeType() != null ) {
            mimeTrace(
              "Content generator is trying to set response mime type to [{0}], but mime type [{1}] has already been "
                + "imposed. Content generator request to change mime type will be ignored.",
    ...
    Map<String, IParameterProvider> parameterProviders = getParameterProviders();
    ...
    IPentahoUrlFactory urlFactory =
      new SimpleUrlFactory( PentahoRequestContextHolder.getRequestContext().getContextPath()
        + "api/repos/" + pluginId + "/" + contentGeneratorID + "?" ); //$NON-NLS-2$ //$NON-NLS-3$
  ...
  @NonNull
  private Map<String, IParameterProvider> createParameterProviders() throws IOException {
