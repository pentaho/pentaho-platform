    ...
    try ( ZipInputStream zipInputStream = new ZipInputStream( inputStream ) ) {
      ...
      while ( entry != null ) {
        ...
        if ( !isDir ) {
          ...
          if ( !fileService.isValidDecodedFileName( decodedEntryName ) ) {
          ...
          if ( !fileService.isValidDecodedFolderName( decodedEntryName ) ) {
            getLogger().error( Messages.getInstance().getString( "DefaultImportHandler.ERROR_0012_INVALID_FOLDER_NAME", decodedEntryName ) );
            throw new PlatformImportException(
