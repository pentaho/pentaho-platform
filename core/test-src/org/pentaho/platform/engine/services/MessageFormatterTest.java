...
public class MessageFormatterTest {
  ...
  @Before
  public void before() {
    mockedRuntimeRepository = Mockito.mock( IRuntimeRepository.class );
    mockedSolutionEngine = Mockito.mock( ISolutionEngine.class );
    ...
    runtimeCtx = Mockito.spy( new RuntimeContext( "id", mockedSolutionEngine, "solutionName",
  ...
  public void formatSuccessMessage() throws Exception {
    ...
    Mockito.when( runtimeCtx.getOutputNames() ).thenReturn( inputNames );
    Mockito.doReturn( actionParameter ).when( runtimeCtx ).getOutputParameter( Mockito.anyString() );
    ...
    Assert.assertEquals( "<html><head><title>Pentaho BI Platform - Start Action</title><link rel=\"stylesheet\" "
  ...
  private Map<String, IParameterProvider> makeParameterProviders( final IPentahoSession session ) {
    final Map<String, IParameterProvider> res = new HashMap<String, IParameterProvider>();
