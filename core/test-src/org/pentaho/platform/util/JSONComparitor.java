...
public class JSONComparitor {
	 ...
	 * @param compareKey			root key to start compare, if null compare the entire string.
	 * @return								true if equivalent, false if different.
	 * @throws JSONException	
	 */
	public static boolean jsonEqual(String json1, String json2, String compareKey)
			throws JSONException {
	 ...
	 * @throws JSONException	
	 */
	public static boolean jsonEqual(JSONObject jso1, JSONObject jso2,
			String compareKey) throws JSONException {
	 ...
	 * @param jsan1 					First JSONArray
	 * @param jsan2 					Second JSONArray
	 * @param item						Index of array to compare, null compare entire array
	 * @return								true if equivalent, false if different.
	 * @throws JSONException	
	 */
	public static boolean jsonEqual(JSONArray jsa1, JSONArray jsa2, Integer item)
			throws JSONException {
	...
	private static boolean compareKey(JSONObject jso1, JSONObject jso2, String key)
			throws JSONException {
		KeyType keyType = getKeyType(jso1, key);
		if (jso2.has(key)) {
			if (keyType == getKeyType(jso2, key))
				if (keyType == KeyType.OBJECT) {
					return jsonEqual(jso1.getJSONObject(key), jso2.getJSONObject(key),
							null);
				} else {
					if (keyType == KeyType.ARRAY) {
						return jsonEqual(jso1.getJSONArray(key), jso2.getJSONArray(key),
								null);
					} else {
	...
	private static boolean compareArray(JSONArray jsa1, JSONArray jsa2, int i)
			throws JSONException {
