...
public class XmlHelperTest extends TestCase {
  ...
  /**
   * Load an XML into a dom4j.Document, convert that document to a string, load that string into a w3c.Document, and
  ...
  public void testEncodeDecode() {
    ...
    assertEquals( "Error encoding after decoding", encodedXml,
      XmlHelper.encode( XmlHelper.decode( encodedXml ) ) ); //$NON-NLS-1$
    assertEquals( "Error decoding after encoding", decodedXml,
      XmlHelper.decode( XmlHelper.encode( decodedXml ) ) ); //$NON-NLS-1$
  }
  ...
  public void testFailureTransformXML() throws Exception {
    ...
    parameters.put( "displayUrl",
      displayUrl ); //$NON-NLS-1$
  ...
  public void testFailureTransformXML2() throws Exception {
    ...
    String xmlString =
      "<?xml version=\"1.0\" encoding=\"" + "UTF-8" + "\" ?><filters xmlns:xf=\"http://www.w3.org/2002/xforms\">"
