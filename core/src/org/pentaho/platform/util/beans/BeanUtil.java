 ...
 * This is not an attempt to duplicate the behavior of commons-beanutils, rather, a central 
 * spot for common operations on beans so we can ensure that same bean property binding functionality
 * and logic anytime we need to work with Java Beans.  
 * <p>This utility is especially important
 * in dealing with Pentaho Action beans {@link IAction}s.  See {@link ActionHarness} for an IAction-specific
...
public class BeanUtil {
   ...
   * @param targetBean the bean on which to operate
   */
  ...
  /**
   * Returns <code>true</code> if a bean property can be written to (i.e. there is an accessible
   * and appropriately typed setter method on the bean).  Note: this method will check for both
   * scalar (normal) and indexed properties before returning <code>false</code>.
   * @param propertyName the name of the bean property to check for write-ability
   ...
   * then the value will be derived by calling {@link ValueGenerator#getValue(String)}
   * 
   * @throws Exception if there was a problem setting the value on the bean
   * @see ValueGenerator
   */
  ...
  public void setValue(String propertyName, Object value, ValueSetErrorCallback callback,
    ...
    if(value == null) {
      ...
      logger.info(MessageFormat.format("value to set is null, skipping setting of \"{0}\" property on bean \"{1}\"",propertyName, bean)); //$NON-NLS-1$
      return;
    }
  ...
  /**
   * Sets a number of bean properties based on given property-value map, where the key of the 
   * map is the bean property and the value is the value to which to set that property.
   * @param propValueMap  a map whose keys are property names and whose values are to be
