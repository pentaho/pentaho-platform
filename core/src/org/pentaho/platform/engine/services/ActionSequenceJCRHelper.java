...
public class ActionSequenceJCRHelper {
  ...
  public ActionSequenceJCRHelper(IPentahoSession pentahoSession) {
    ...
    if (repository == null) {
      final String errorMessage = Messages.getInstance().getErrorString("ActionSequenceJCRHelper.ERROR_0001_INVALID_REPOSITORY"); //$NON-NLS-1$
      ...
      while (nodeIterator.hasNext()) {
        ...
        if (name.startsWith("%") && !node.getPath().endsWith("/text()")) { //$NON-NLS-1$ //$NON-NLS-2$
          try {
            ...
            logger.warn(Messages.getInstance().getString("ActionSequenceJCRHelper.WARN_MISSING_RESOURCE_PROPERTY", name.substring(1), baseName, getLocale().toString())); //$NON-NLS-1$
          }
        }
      }
    } catch (Exception e) {
      logger.error(Messages.getInstance().getErrorString("ActionSequenceJCRHelper.ERROR_0007_COULD_NOT_READ_PROPERTIES", file.getPath()), e); //$NON-NLS-1$
    }
  }
  ...
  protected String getLocaleString(final String key, String baseName, final RepositoryFile baseFile, boolean marchUpParents) {
    ...
    try {
      ...
      logger.error(Messages.getInstance().getErrorString("ActionSequenceJCRHelper.ERROR_0007_COULD_NOT_READ_PROPERTIES", baseFile.getPath()), e); //$NON-NLS-1$
    }
    return null;
  }
