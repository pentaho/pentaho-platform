...
public class PooledDatasourceSystemListener implements IPentahoSystemListener {
  ...
  public boolean startup( final IPentahoSession session ) {
     ...
     * PentahoSystem.get(IDatasourceMgmtService.class,session);
     ...
     * datasourceMgmtSvc.getDatasources(); for (DatabaseMeta datasource : datasources) { try { Logger.debug(this,
     * "  Setting up pooled Data Source - " + datasource); //$NON-NLS-1$ final DataSource ds =
     * PooledDatasourceHelper.setupPooledDataSource(datasource); Logger.debug(this, "(storing DataSource under key \"" +
     * IDBDatasourceService.JDBC_DATASOURCE //$NON-NLS-1$ + datasource.getName() + "\")"); //$NON-NLS-1$
     * cacheManager.putInRegionCache(IDBDatasourceService.JDBC_DATASOURCE, datasource.getName(), ds); } catch
     * (DBDatasourceServiceException dse) { // Skip this datasource pooling Logger.error(this,
     * Messages.getInstance().getErrorString
     * ("PooledDatasourceSystemListener.ERROR_0003_UNABLE_TO_POOL_DATASOURCE",datasource.getName(), dse.getMessage()));
     * //$NON-NLS-1$ continue; } } Logger.debug(this, "PooledDatasourceSystemListener: done with init"); //$NON-NLS-1$
     */
    return true;
    /*
     * } catch (DatasourceMgmtServiceException dmse) { Logger.error(this,
     * Messages.getInstance().getErrorString("PooledDatasourceSystemListener.ERROR_0002_UNABLE_TO_GET_DATASOURCE"),
     * dmse); //$NON-NLS-1$ return false; }
     */
  }
