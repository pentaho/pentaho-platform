...
/**
 * This formatter exists to make old-style action sequence inputs,
 * outputs and resource names work with bean utils which adheres
...
public class ActionSequenceCompatibilityFormatter implements PropertyNameFormatter {
  ...
  protected String compatibilityToCamelCase(String name) {
    ...
    if (parts.length > 1) {
      ...
      logger.warn(
          Messages.getInstance().getString("ActionSequenceCompatibilityFormatter.WARN_USING_IO_COMPATIBILITY_MODE", camelCaseName, name)); //$NON-NLS-1$
