 ...
 * uses a config attribute prefix which serves a similar, but not the same, purpose. In {@code RoleVoter}, the role
 * prefix serves as an indicator to the {@code RoleVoter} to participate in the voting. {@code RoleVoter} assumes that
 * the config attributes are role names. In this voter implementation, the {@code processConfigAttributePrefix} is
 * stripped from the config attribute before comparison to the roles granted to the user.
 * 
 * <p>
