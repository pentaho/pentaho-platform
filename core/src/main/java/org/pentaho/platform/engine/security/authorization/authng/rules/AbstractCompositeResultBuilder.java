...
public abstract class AbstractCompositeResultBuilder {
  ...
  public AbstractCompositeResultBuilder( @NonNull AuthorizationDecisionReportingMode reportingMode ) {
  ...
  public void withDecision( @NonNull IAuthorizationDecision decision ) {
    ...
    if ( isSettled() ) {
      ...
      // 1.b. SAME_STATUS reporting and different granted status -> ignore
      if ( reportingMode.equals( AuthorizationDecisionReportingMode.SAME_STATUS )
    ...
    //      (regardless of reporting mode: SETTLED, SAME_STATUS, FULL)
    if ( resultDecision.isGranted() == decision.isGranted() ) {
      ...
      // 2.b.2. else (SETTLED, SAME_STATUS reporting) -> replace (granted status and decisions)
      setSingleDecision( decision );
    }
  }
